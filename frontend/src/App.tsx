...
export default function App() {
  ...
  const visibleSolutions = useMemo(() => {
    ...
    return includeStandard
      ? result.solutions
      : result.solutions.filter((solution) => !solution.is_standard);
    ...
    <main className="shell">
      ...
      <nav className="nav">
        ...
        <a
          ...
          href="https://github.com/robert-7/sicherman-dice-generator"
          target="_blank"
          rel="noreferrer"
        >
      ...
      <section className="hero" id="top">
        <div className="eyebrow">
          <Sparkles size={15} /> same probabilities, wonderfully strange labels
        </div>
        <h1>
          Roll ordinary odds with <span>extraordinary dice.</span>
        </h1>
        <p>
          Generate every positive-integer relabeling of equally-sided dice that preserves the exact
          sum distribution of the ordinary set.
        </p>
      </section>
      ...
      <section className="generator-grid">
        <form className="control-card" onSubmit={generate}>
          <div className="section-heading">
            <div>
              ...
              <span>Configure the experiment</span>
            </div>
        ...
        <aside className="classic-card">
          ...
          <div className="same-odds">
            <CheckCircle2 size={17} /> Same 36 sum outcomes
          </div>
        </aside>
      </section>
        ...
        <section className="results">
          <div className="results-header">
            <div>
              ...
              <h2>
                {result.non_standard_solution_count === 1
                  ? "1 non-standard solution"
                  : `${result.non_standard_solution_count} non-standard solutions`}
              </h2>
              <p>
                {result.candidate_die_count} legal die polynomials were tested for {result.dice}D
                {result.faces}.
              </p>
            </div>
          ...
          {visibleSolutions.length === 0 ? (
            <div className="empty-state">
              ...
              <p>
                The ordinary D{result.faces} set is the only positive-label solution for{" "}
                {result.dice} dice.
              </p>
            </div>
          ) : (
                ...
                <article className="solution-card" key={solutionIndex}>
                  <div className="solution-title">
                    ...
                    {solution.is_standard ? (
                      <span className="standard-pill">ordinary</span>
                    ) : (
                      <span className="weird-pill">Sicherman-equivalent</span>
                    )}
                  </div>
                  <div className="dice-grid">
                    {solution.dice.map((die, dieIndex) => (
                      <DieCard key={dieIndex} die={die} index={dieIndex} />
                    ))}
                  </div>
                </article>
              ))}
          ...
          >
            <span>
              <Atom size={18} /> Why does this work?
            </span>
          ...
          {showMath && (
            <div className="math-panel">
              ...
              <p>
                For {result.dice} dice, every factor appears {result.dice} times. The generator
                redistributes those irreducible factors among the dice, then keeps only products
                with non-negative integer coefficients, exactly {result.faces} total faces, and no
                zero-valued face. Matching the product means matching every coefficient of the
                sum-distribution generating function.
              </p>
            </div>
          )}
        </section>
      ...
      <footer>
        <span>
          Built from generating functions, cyclotomic polynomials, and a little mathematical
          mischief.
        </span>
